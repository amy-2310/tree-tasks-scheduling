...
{-
The first list is used to memorize operations in the same state of a schedule. When a local schedule is transformed into a linear pseudo-process, each state of the schedule is transformed into a pseudo-operation memorizing all operations in that state.
The second list contains the sub-processes generated by the operation (op).
The third list contains the predecessors of the op, which were removed durng standardization. The op is a local peak in one of the four patterns. These predecessors will at the end be inserted back into the resulting schedule.
The forth list contains the removed successor of the op... (analog to the third list).
-}
...
It takes an input-program and a list of relations (the data type Relation is declared in module TopoSort).
It first calls toProgram to convert the two arguments into a Program (recursive data type).
Then, it calls spoptnr, which delivers a schedule (in the form of a linear pseudo-process) of the program. Lastly, it calls revcover, which converts the linear pseudo-process into a schedule.
-}
...
{-
print_spopt_schedule works almost the same. However, the resulting schedule will be printed one state per line in ghci.
-}
...
It first converts all inpue-processes to data type Process and sorts the relation list by calling the help-functions toProcess and tpop_sort respectively.
Then, it calls the help-function toProgram_h, which goes through the relation list recursively and inserts sub-processes into a specific operation according to a relation in each round. Lastly, the resulting process-list (Program) is sorted by process-id and deliverd.
-}
...
toProgram_h ls ((Rel pid oid ps):r_rest)
        = let --the main-process with pid
              main_p = fromJust $ find (\p -> (get_pid $ head p) == pid) ls
              
              --the sub-processes to be inserted into the main-process with pid
              sub_ps = filter (\p -> elem (get_pid $ head p) ps) ls
              
              --other processes in the process-list
              other_ps = filter (\p -> (not $ elem (get_pid $ head p) ps) &&
                                       (not $ (get_pid $ head p) == pid)
                                ) ls
              
               --insert sub-processes into the main-process
