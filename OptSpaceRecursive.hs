...
The third list contains the predecessors of the op, which were removed durng
standardization. The op is a local peak in one of the four patterns. These
...
relation list by calling the help-functions toProcess and tpop_sort respectively.
...
toProgram_h ls ((Rel pid oid ps):r_rest)
        = let --find the process with pid
              main_p = fromJust $ find (\p -> (get_pid $ head p) == pid) ls
              
              --find the processes to be inserted into the process with pid
              ...
              other_ps = filter (\p -> (not $ elem (get_pid $ head p) ps) &&
               ...
               --insert processes in to the process with pid
